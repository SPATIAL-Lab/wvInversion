model {
  ...
  for(i in 2:nt){
    ...
    evap_r[i] = (r_o[i-1, 1] / evap_a[i] - rh[i] * r_bl[i]) / 
      ((1 - rh[i]) * 1.028489)
    ...
    r_bl[i] = (r_bl.del[i] / 1000 + 1) * 0.0020052
    r_bl.del[i] ~ dnorm(r_bl_pri[i, 1], r_bl_pri[i, 2])
  ...
  psi_s ~ dnorm(-0.62, 1/0.02^2)I(-0.75, -0.5)
  #psi_s = -0.62 
}